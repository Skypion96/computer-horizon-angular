...
export class OrdinateurPipe implements PipeTransform {
  ...
  transform(ordiList: OrdiList, filter: string,typePrix:Prix = Prix.ALL,typeMarque:Marques=Marques.ALL,typeSSD:DisqueDSsd=DisqueDSsd.ALL): any {
    ...
    else if(typeSSD==DisqueDSsd.AVEC){
      if(!filter){
        if(typePrix == Prix.ALL && typeMarque==Marques.ALL){
